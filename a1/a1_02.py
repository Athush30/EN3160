...
#gaussian pulse for grey matter
mu=200
sigma=10
x = np.linspace(0, 255, 256)
gaussian_grey = 255 * np.exp(-((x - mu)**2) / (2 * sigma**2))

# Ensure the output is in the valid range for image intensities [0, 255]
gaussian_grey = np.clip(gaussian_grey, 0, 255)

print(gaussian_grey.shape)

#plot the array
plt.figure(figsize=(5, 5))
plt.plot(gaussian_grey)
plt.xlabel("Input intensity")
plt.xlim(0, 255)
plt.ylim(0, 255)
plt.ylabel("Output intensity")
